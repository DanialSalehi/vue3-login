<template>
  ...
  >
    ...
    <div class="form-container sign-in-container">
      <form action="#">
        <div class="social-container">
          <h1>Sign {{ loged ? 'Out' : 'In' }}</h1>
        </div>

        <span v-if="!loged">enter by your account</span>
        <input
          v-if="!loged"
          type="username"
        ...
        />
        <br />
        <div v-if="!loged" class="button" @click="login">Sign In</div>
        <button v-if="loged" class="button" @click="logout">Sign Out</button>
      </form>
    </div>
...
.overlay {
  background: #ff416c;
  background: -webkit-linear-gradient(to right, #0a9196, #4184ff);
  background: linear-gradient(to right, #0a9196, #4184ff);
